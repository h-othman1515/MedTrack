...
$(document).ready(function() {
    ...
    // Initialize DataTables (skip colspan placeholder rows — they break column count)
    if ($.fn.DataTable) {
...
function buildDashboardCharts() {
    ...
    if (ctx1) {
        var inventoryData = window.mtInventoryChartData || [0, 0, 0, 0];
    ...
    if (ctx2) {
        var trend = window.mtExpiryTrendData || { labels: [], waste: [], savings: [] };
        dashboardCharts.push(new Chart(ctx2, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: trend.waste.length ? trend.waste : [0],
                    borderColor: '#dc3545',
                    ...
                    data: trend.savings.length ? trend.savings : [0],
                    borderColor: '#198754',
    ...
    if (ctx3) {
        var shortage = window.mtShortageChartData || { labels: [], active: [], resolved: [] };
        dashboardCharts.push(new Chart(ctx3, {
            type: 'bar',
            data: {
                labels: shortage.labels.length ? shortage.labels : ['—'],
                datasets: [{
                    label: 'Active Shortages',
                    data: shortage.active.length ? shortage.active : [0],
                    backgroundColor: '#dc3545'
                }, {
                    label: 'Resolved This Month',
                    data: shortage.resolved.length ? shortage.resolved : [0],
